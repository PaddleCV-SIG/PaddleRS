...
运行一下命令进行SNUNET模型训练，在训练过程中会对模型进行评估，启用了VisualDL日志功能，运行之后在`/output/snunet/vdl_log` 文件夹下找到对应的日志文件

```shell
python tutorials/train/change_detection/snunet_train.py --data_dir=../work/Real/subset --out_dir=./output/snunet/
```
...
```shell
python tutorials/eval/change_detection/snunet_eval.py --data_dir=../work/Real/subset --weight_path=./output/snunet/best_model/model.pdparams
```
**参数介绍**：
...
```
### 模型预测
本项目提供预测脚本`tutorials\predict\change_detection\snunet_pred.py`，设置以下参数就可直接运行
- weight 训练好的权重
- A,B, 是T1影像路径,T2影像路径
- pre 预测图片存储的位置

```shell
python tutorials/predict/change_detection/snunet_pred.py --weight=./output/snunet/best_model/model.pdparams --A=../work/Real/subset/test/A/00002.jpg --B=../work/Real/subset/test/B/00002.jpg \
--pre=../work/pre.png
```
